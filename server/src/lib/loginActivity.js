 ...
 */
async function recordLoginActivity({ req, emailAttempted, status, userId = null, }) {
    try {
        ...
        if (status === 'SUCCESS' && userId) {
            ...
            const priorSuccesses = await prisma_1.default.loginActivity.findMany({
            ...
            const lastSuccess = await prisma_1.default.loginActivity.findFirst({
                where: { emailAttempted: email, status: 'SUCCESS' },
                orderBy: { createdAt: 'desc' },
            });
            failedAttemptsBeforeSuccess = await prisma_1.default.loginActivity.count({
                where: {
        ...
        await prisma_1.default.loginActivity.create({
            data: {
                emailAttempted: email,
                userId,
                status,
