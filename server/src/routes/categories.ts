      ...
      if (data.parentId) {
        ...
        if (parent.type !== data.type) {
          return res.status(400).json({
            error:
              'Subcategory type must match the parent category type.',
          });
        }
        ...
        if (parent.parentId) {
          return res.status(400).json({
            error:
              'Subcategories cannot have another subcategory.',
          });
        }
      }
    ...
    try {
      ...
      if (
        ...
        if (childrenCount > 0) {
          return res.status(400).json({
            error:
              'Cannot change the type of a category that has subcategories.',
          });
        }
      }
  ...
  async (req, res: Response, next) => {
    try {
      ...
      if (existing.children.length > 0) {
        return res.status(400).json({
          error:
            'Cannot delete this category because it still has subcategories. Please delete or move the subcategories first.',
        });
      }
